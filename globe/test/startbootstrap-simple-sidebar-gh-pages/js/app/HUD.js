 ...
 * HUD
 */
define(["jquery", "putils"], function ($, putils) {
	...
	function HUD( container ) {
		...
		this._gui.id = "sidebar-wrapper"; 
		this._gui.className = "";
		this._gui.innerHTML = '<a href="#" id="menu-toggle2" class="arrow bounceleft"> \
		<span class="panohidden2"><i class="fas fa-angle-right"></i></span>&nbsp; \
		</a>';
		
		this._ul = document.createElement("ul");
		this._ul.className = "sidebar-nav";
		this._gui.appendChild( this._ul );
...
// collapse.appendChild(listGroup);
		...
		function buildList( index ) {
			...
			listGroup.appendChild(li);
			li.innerHTML = citys[index].adresse + " (" + putils.numberWithCommas( Math.floor( citys[ index ].hopDistance ) ) + " km)"
		}
