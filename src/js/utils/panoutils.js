...
export const calc3DPositions = (data, heightData, radius) => {
  ...
  for (let i = 0; i < data.length; i += 1) {
    ...
    if (heightData.length > 0) {
      const latHeight = Math.floor(((data[i].lat * (heightData.length / 180)) - (heightData.length / 2)) * -1);
      const lngHeight = Math.floor((data[i].lng) * (heightData[0].length / 360)) + (heightData[0].length / 2);
